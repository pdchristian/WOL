...
/* Remotedesktop: nativer Versuch via ms-rd:// (Microsoft Remote Desktop).
...
function doRemote(id, mode) {
  Native.call("remote", { id, mode: mode === "win" ? "win" : "full" }).then(res => {
    if (!res.ok) {
      ...
      toast(msg === "remote.notinstalled" ? t("remote.notinstalled") : msg, true);
    }
  });
}
...
function renderSettings() {
  ...
  $("#s-set").innerHTML = `
    ...
    <div class="aboutBlock">
      ...
      <div class="aboutVer">${esc(t("about.version", { v: APP_VERSION }))} · iOS</div>
    ...
    case "set-reset":
      openConfirm(t("set.reset.title"), t("set.reset.msg"), () => {
        Native.call("resetSettings", {}).then(res => {
          if (!res.ok) { toast(String(res.error || "error"), true); return; }
          Native.call("snapshot", {}).then(s => { applySnapshot(s.data); closeSheet(); renderAll(); });
        });
      });
      break;
    /* ── Info / Update ── */
