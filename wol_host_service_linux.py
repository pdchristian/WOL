...
CLI usage (run with sudo for install/uninstall/start/stop):
    ...
    wol_host_service.py --network-gate on|off  Public-network gate (default on)
    wol_host_service.py --allow-public on|off  Privileged commands on public networks
...
#
# On a public network (firewalld zone "public" - hotel/conference Wi-Fi,
# ...) the machine may be reachable from strangers, so the privileged
# one-shot commands (shutdown / reboot / run_batch) are refused even with
# valid credentials - defense in depth. status/metrics stay available.
#
# Controlled on the host:
#   --network-gate on|off    enable/disable the check (default on)
#   --allow-public on|off    permit privileged commands despite "public"
# Detection failures (no firewalld, unknown zone) do NOT block - only an
# explicitly public zone does.
...
_net_cache: tuple[float, int] = (0.0, CATEGORY_UNKNOWN)
...
def allow_in_public_network() -> bool:
    """Per-machine override: privileged commands despite a public zone."""
...
def _network_category() -> int:
    """Network category from firewalld's active zones (cached, best effort).

    All active zones "public" -> PUBLIC; any other (trusted) zone ->
    PRIVATE; firewalld absent/inactive -> UNKNOWN (never blocks).
    """
    global _net_cache
    now = time.monotonic()
    with _net_cache_lock:
        ...
        try:
            ...
            if state.returncode == 0 and state.stdout.strip() == "running":
                ...
                parts = zones.stdout.replace("active zones:", " ").split()
                if parts:
...
def network_gate_check(command: str) -> str | None:
    """Reject privileged commands while the host sits on a PUBLIC zone.

    Returns an error message to respond with, or ``None`` when the command
    may proceed. Only an explicitly public zone blocks; unknown/undetectable
    profiles never do (availability over paranoia).
    """
    if command not in NETWORK_GATE_COMMANDS:
        return None
    if not network_gate_enabled() or allow_in_public_network():
        return None
    if _network_category() == CATEGORY_PUBLIC:
...
class _CommandHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self) -> None:  # noqa: N802 (socketserver API)
        try:
            ...
            # host sits on a public firewalld zone (read-only mode).
