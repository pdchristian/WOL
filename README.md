...
**Version 2.3.4 - Service Watch Edition**
...
#### 📱 iOS app (`ios/`)
- **Remote Desktop now launches the Windows App:** the *Remote fullscreen* / *Remote window* actions (device tiles and context menu) open the installed **Windows App** (formerly Microsoft Remote Desktop) with a ready-made profile — PC address (device IP, falling back to the device name) and username are pre-filled via URI (`rdp://full%20address=s:…&username=s:…`, with `ms-rd://add/host/…` as fallback candidate), and the profile **stays** in the Windows App (no clean-up, unlike the desktop app). No iOS URI scheme can carry a password, so the stored device password is placed on the clipboard (paste into the connect window, toast hint) and reported as `passwordCopied`. Missing app or missing address report a localised error (DE/EN/FR/ES). URI building lives in `Util/RemoteDesktop.swift` (pure builder, covered by `RemoteDesktopTests`); `Info.plist` lists `rdp` + `ms-rd` under `LSApplicationQueriesSchemes`
