...
enum class ShutdownMethod {
    ...
    companion object {
        fun from(s: String?): ShutdownMethod =
            if (s?.equals("smb", ignoreCase = true) == true) SMB else HOST_SERVICE
    }
}
 ...
 * verschlüsselt in [de.wolmanager.data.SecureStore] abgelegt. [id] und die Runtime-Felder
...
data class Device(
    ...
    val method: ShutdownMethod get() = ShutdownMethod.from(shutdownMethod)
}
