...
Expected: `dist/wake-on-lan-manager_2.3.6-1_all.deb`.

## 3. Install

```bash
sudo apt install ./dist/wake-on-lan-manager_2.3.6-1_all.deb
```
...
- Tag `v2.3.6`, build the `.deb`, attach it to the GitHub Release.
- Install command in the release notes:
  `sudo apt install ./wake-on-lan-manager_2.3.6-1_all.deb`
